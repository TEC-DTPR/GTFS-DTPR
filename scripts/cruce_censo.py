...
def process_zone(slug, manz_paths):
    ...
    # ---- manzanas.geojson liviano (solo las servidas, para la capa coroplética) ----
    try:
        export_manzanas(slug, manz, mask_zona)
    except Exception as e:
        log(f"[{slug}]   (aviso) no se exportó manzanas.geojson: {e}")
...
def export_manzanas(slug, manz, mask):
    """Exporta las manzanas dentro del buffer, en 4326, con solo los campos que
    se pintan en el mapa (población, grupos etarios, modos). Geometría simplificada."""
    sel = manz[mask].copy()
    if sel.empty:
        return
    keep = ["n_per"] + [c for c in POB_FIELDS if c.startswith("n_edad") or c.startswith("n_transp")]
    keep = [c for c in dict.fromkeys(keep) if c in sel.columns]
    sel = sel[keep + ["geometry"]].to_crs(CRS_GEO)
    # simplifica ~8 m en grados aprox (0.00008) para bajar peso
    sel["geometry"] = sel.geometry.simplify(0.00008, preserve_topology=True)
    for c in keep:
        sel[c] = sel[c].round(0).astype(int)
    path = DATA_DIR / slug / "manzanas.geojson"
    sel.to_file(path, driver="GeoJSON")
    mb = path.stat().st_size / 1e6
    log(f"[{slug}]   manzanas.geojson · {len(sel):,} manz · {mb:.1f} MB")
...
def run():
    ...
    for slug, paths in sorted(grupos.items()):
        try:
            r = process_zone(slug, sorted(paths))
            if r:
                ok += 1
                z = json.load(open(DATA_DIR / slug / "censo.json", encoding="utf-8"))["zona"]
                modos = sum(z.get(k, 0) for k in ["n_transpor", "n_transp_1", "n_transp_2",
                            "n_transp_3", "n_transp_4", "n_transp_5", "n_transp_6"]) or 1
                resumen[slug] = {
                    "n_per": z.get("n_per", 0), "n_hombres": z.get("n_hombres", 0),
                    "n_mujeres": z.get("n_mujeres", 0),
                    "pct_tp": round(100 * z.get("n_transp_1", 0) / modos, 1),
                }
        except Exception as e:
            log(f"[{slug}] ERROR censo: {e}")

    # inyecta el resumen de poblacion en index.json (para la tabla del sidebar)
    idx_path = DATA_DIR / "index.json"
    if idx_path.exists() and resumen:
        idx = json.load(open(idx_path, encoding="utf-8"))
        for z in idx.get("zones", []):
            if z.get("slug") in resumen:
                z["censo"] = resumen[z["slug"]]
