...
def process_feed(name, slug, g):
    ...
    # ------------------------------------------------------------------ #
    def route_metrics_for_day(svc_set):
        """dict route_id -> (expediciones, veh_km, vector24 salidas) para un weekday."""
        sub = trips[trips["service_id"].isin(svc_set)]
        out = {}
        if sub.empty:
            return out
    ...
    # métricas por weekday (0..6)
    if sdays is not None:
        wd_metrics = [route_metrics_for_day(sdays[WD_KEYS[wd]]) for wd in range(7)]
    else:
        allm = route_metrics_for_day(set(trips["service_id"]))
        wd_metrics = [allm] + [{}] * 6

    def route_promedio(rid, wd_indices):
        """Promedia ponderado por días del mes para un route en los weekday dados."""
        acc_vec = [0.0]*24; acc_km = 0.0; acc_exp = 0.0; ndias = 0
        for wd in wd_indices:
            cnt = per_wd[wd]
            if cnt == 0:
                continue
            e, km, vec = wd_metrics[wd].get(rid, (0, 0.0, [0]*24))
            for h in range(24):
                acc_vec[h] += vec[h] * cnt
    ...
    dur_by_route = trips.dropna(subset=["_dur"]).groupby("route_id")["_dur"].mean().to_dict()

    # km de trazado por route, separando ida / regreso por direction_id
    shapes_by_route = {}
    for sid, meta in shp_meta.items():
        shapes_by_route.setdefault(meta.get("route_id", ""), []).append(sid)

    def km_ida_regreso(rid):
        ida = reg = 0.0
    ...
    for rid, info in rinfo.items():
        freq_lab, km_lab, exp_lab = route_promedio(rid, [0, 1, 2, 3, 4])
        freq_sat, km_sat, exp_sat = route_promedio(rid, [5])
        freq_dom, km_dom, exp_dom = route_promedio(rid, [6])

        km_mes = km_lab * mdays["laboral"] + km_sat * mdays["sabado"] + km_dom * mdays["domingo"]
        km_ida, km_reg = km_ida_regreso(rid)
        km_trazado = round(km_ida + km_reg, 2)

        services[short_of(rid)] = {
            "servicio": short_of(rid),
            "nombre": info.get("route_long_name", ""),
            "color": color_of(rid),
            "km_trazado": km_trazado,
            "km_ida": km_ida,
            "km_regreso": km_reg,
            "km_mensual": round(float(km_mes), 0),
            "veh_km_dia": {"laboral": km_lab, "sabado": km_sat, "domingo": km_dom},
            "expediciones": {"laboral": exp_lab, "sabado": exp_sat, "domingo": exp_dom},
            "t_viaje_min": round(float(dur_by_route.get(rid, np.nan)), 1) if rid in dur_by_route else None,
            "freq": {"laboral": freq_lab, "sabado": freq_sat, "domingo": freq_dom},
        }
    ...
    km_mes_zona = round(sum(s["km_mensual"] for s in services.values()), 0)
