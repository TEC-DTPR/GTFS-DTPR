...
L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
  attribution: '&copy; OpenStreetMap · GTFS DTPR · STP Regional', maxZoom: 19,
}).addTo(map);
...
function ensureDemMap() {
  ...
  L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19 }).addTo(dem.map);
}
...
function renderHexHeat() {
  ...
  const R = d.hex_r_m;
  // un solo renderer canvas para todos los hexágonos (no se pega con miles)
  ...
  acc.forEach((val, hi) => {
    ...
    feats.push(hexPoly(lat, lon, R, val / max, val, renderer));
...
function hexPoly(lat, lon, R, t, val, renderer) {
  ...
  return L.polygon(pts, { renderer, color: '#94A0B0', weight: 0.3, opacity: .25, fillColor: choroColor(t), fillOpacity: val > 0 ? .82 : .1 })
    .bindTooltip(`${fmt(Math.round(val))} pax`, { className: 'stop-tip', sticky: true });
}
