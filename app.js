...
/* escala de color secuencial gris→naranjo→rojo para el coroplético */
function choroColor(t) {
  t = Math.max(0, Math.min(1, t));
  const stops = [[42, 49, 60], [255, 158, 27], [200, 16, 46]]; // #2A313C, naranjo, rojo
  const seg = t < 0.5 ? 0 : 1;
...
L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', {
  attribution: '&copy; OpenStreetMap &copy; CARTO · GTFS DTPR · Red Movilidad', subdomains: 'abcd', maxZoom: 20,
}).addTo(map);
...
function drawStops(geojson) {
  ...
  state.layers.stops = L.geoJSON(geojson, {
    pointToLayer: (f, ll) => L.circleMarker(ll, { radius: 4, weight: 1.4, color: '#0E1116', fillColor: '#C8102E', fillOpacity: 1 }),
    onEachFeature: (f, layer) => {
      const p = f.properties;
...
function selectService(ss) {
  ...
  if (state.layers.stops) state.layers.stops.eachLayer((l) => {
    ...
    l.setStyle(on ? { radius: 5, fillColor: '#FF9E1B', fillOpacity: 1, opacity: 1 } : { radius: 2.5, fillColor: '#3a4250', fillOpacity: .5, opacity: .4 });
  ...
  openService(ss);
  renderCenso(ss);
  // gráfico grande pasa a mostrar la frecuencia del servicio
  const sd = (state.services || {})[ss];
  $('#freq-title').textContent = `Frecuencia · ${ss}`;
  renderChart(sd?.freq || state.freq, true);
}
...
function openService(ss) {
  ...
  if (d) {
    ...
    const cards = [
      { l: 'Km de trazado', v: fmt1(d.km_trazado), u: 'km', f: `ida ${fmt1(d.km_ida ?? 0)} · regreso ${fmt1(d.km_regreso ?? 0)}` },
...
function renderCenso(scope) {
  ...
  $('#censo-scope').textContent = (isZona ? 'Zona' : `Servicio ${scope}`) + ` · buffer ${c.buffer_m || 200} m`;
...
function renderSvChart(d) {
  ...
  if (state.svChart) { state.svChart.data.datasets[0].data = data; state.svChart.data.datasets[0].backgroundColor = d.color || '#C8102E'; state.svChart.update(); return; }
  state.svChart = new Chart($('#sv-chart'), {
    type: 'bar',
    data: { labels, datasets: [{ data, backgroundColor: d.color || '#C8102E', borderRadius: 2, barPercentage: .95, categoryPercentage: .9 }] },
    options: { responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: { ...TT, bodyColor: '#FF9E1B',
        callbacks: { title: (i) => `${i[0].label}:00–${i[0].label}:59 h`, label: (i) => [` ${fmt1(i.parsed.y)} salidas`, ` headway ${headwayLabel(i.parsed.y)}`] } } },
      scales: { x: { grid: { display: false }, ticks: { ...AXIS, maxRotation: 0, autoSkip: true, maxTicksLimit: 12 } },
...
async function mapChoro(kind, field, label) {
  ...
  state.layers.manz = L.geoJSON(gj, {
    ...
    style: (f) => {
      ...
      return { fillColor: choroColor(v / max), fillOpacity: v > 0 ? .72 : .12, weight: 0.3, color: '#0E1116', opacity: .5 };
    },
    onEachFeature: (f, layer) => {
      const v = +f.properties[field] || 0;
...
$('#daychips').addEventListener('click', (e) => {
  ...
  renderMetrics();
  if (state.sel) {
    renderChart(state.services[state.sel]?.freq || state.freq, true);
